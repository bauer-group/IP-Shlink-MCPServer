"""
Shlink MCP Server - Structured logging
...
def setup_logging(log_format: str = "console", log_level: str = "INFO") -> None:
    ...
    # Processors that run for both structlog-native and stdlib-routed records.
    ...
    # Stdlib root logger - third-party libs land here and get reformatted.
    ...
    # Replace any default handlers (uvicorn installs its own).
...
def get_logger(name: str | None = None) -> structlog.stdlib.BoundLogger:
    ...
    return structlog.stdlib.get_logger(name or "bg-shlink-mcp")
...
def print_banner(*, version: str, environment: str, auth_mode: str, public_base_url: str) -> None:
    """Pretty boot banner - safe to call before setup_logging finished."""
    console.print(
        f"\n[bold cyan]Shlink MCP Server[/bold cyan] [dim]v{version}[/dim]\n"
