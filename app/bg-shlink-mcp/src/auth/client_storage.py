...
FastMCP's OAuth providers (AzureProvider, GoogleProvider, OIDCProxy,
OAuthProxy) persist five classes of state:
...
* AUTH_REDIS_URL set
  → RedisStore wrapped in FernetEncryptionWrapper using the operator-
    ...
    supports horizontal scaling (multiple shlink-mcp replicas share
    one Redis).
...
The factory is always called; both branches return a real AsyncKeyValue.
Returning None would let FastMCP fall back to its platformdirs default,
...
logger = structlog.stdlib.get_logger("bg-shlink-mcp.auth.storage")
