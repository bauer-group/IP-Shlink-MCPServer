...
logger = structlog.stdlib.get_logger("bg-shlink-mcp.auth.factory")
...
def build_auth_provider(settings: Settings) -> Any | None:
    ...
    if mode in (AuthMode.ENTRA_SINGLE, AuthMode.ENTRA_MULTI):
        from .entra import build_entra_provider

        return build_entra_provider(settings)

    if mode is AuthMode.GOOGLE:
